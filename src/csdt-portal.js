...
AFRAME.registerComponent('web-portal', {
  ...
  init: function () {
    ...
    if (data.url == 'parent') {
    ...
    } else if (data.enableCSDT == true) {
      //special case for when linking to a CSDT site
    } else {
      //for standard cases
      if (data.enableWebsurface == true) {
        ...
        //if not using websurfaces, create a new iframe
        el.setAttribute('geometry', { primitive: 'plane', width: data.width, height: data.height });
        el.setAttribute('material', { color: '#c8c' });
      ...
      if (data.enableReturnButton == true) {
        const button = (data.returnButton = document.createElement('button'));
